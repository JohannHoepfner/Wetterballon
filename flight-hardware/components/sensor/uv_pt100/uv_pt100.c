...
static const char *TAG = "uv_pt100_sensor";

double get_voltage_photo(int photo_num);
double pt100_get_temp_cels(void);

Sensor uv_pt100_sensor = {
    .init = init_ads,
    .read = get_photo_volt_and_temp,
};
...
double get_photo_volt_and_temp(Sensor *self) {
    UV_PT100_Sensor *uv_pt100 = (UV_PT100_Sensor *)self->ctx;

    if (uv_pt100->type == UV_SENSOR) {
        return get_voltage_photo(uv_pt100->photo_num);
    } else if (uv_pt100->type == PT100_SENSOR) {
        return pt100_get_temp_cels();
    } else {
        return NAN;
    }
}
