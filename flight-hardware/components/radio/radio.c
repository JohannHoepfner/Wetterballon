...
long last_freq = -1;
esp_err_t radio_send_bit(bool bit) {
...
esp_err_t radio_send_bits(bool buf[5]) {
    radio_send_bit(0);
    for (size_t i = 0; i < 5; ++i) {
        radio_send_bit(buf[i]);
    }
    radio_send_bit(1);
    radio_send_bit(1);
    return ESP_OK;
...
esp_err_t radio_send_msg(char *buf, size_t buflen) {
    ...
    radio_send_bits(rtty_symbol_table[37]); // CR
    radio_send_bits(rtty_symbol_table[38]); // LF
    ...
    for (size_t i_char = 0; i_char < buflen; ++i_char) {
        ...
        if (c < 58 && c != 32) {
            ...
            symbolShift = 22;
            if (!numbersModePrev) {
                radio_send_bits(rtty_symbol_table[39]);
            }
        } else {
            // Letters
            numbersMode = false;
            if (numbersModePrev) {
                radio_send_bits(rtty_symbol_table[40]);
            }
        }
        ...
        radio_send_bits(rtty_symbol_table[symbolIndex]);
    }
